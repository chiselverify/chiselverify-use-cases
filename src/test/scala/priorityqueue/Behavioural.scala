...
object Behavioural {
    ...
    def heapifyDown(arr: Array[PriorityType], k: Int, n: Int, i: Int): Unit = {
        var smallest = i
        val children = Seq.tabulate(k)(num => (i * k) + 1 + num)
        for (child <- children) {
            if (child < n && (arr(smallest).c > arr(child).c || (arr(smallest).c == arr(child).c && arr(smallest)(1) > arr(child)(1)))) {
                smallest = child
            }
        }
        if (smallest != i) {
