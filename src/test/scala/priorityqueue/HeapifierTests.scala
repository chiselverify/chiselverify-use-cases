...
import priorityqueue.Behavioural._
import org.scalatest.{FlatSpec, Matchers}
import priorityqueue.modules.Heapifier
...
private class HeapifierWrapper(dut: Heapifier, size: Int, chCount: Int, debugLvl: Int)(cWid: Int, nWid: Int, rWid: Int) extends PeekPokeTester(dut) {
    ...
    poke(dut.io.headPort.rdData.prio.c, root(0))
    poke(dut.io.headPort.rdData.prio.n, root(1))
        ...
        for (i <- 0 until n) {
            ...
            try {
                for (i <- 0 until chCount) {
                    ...
                    poke(dut.io.rdPort.data(i).prio.c, mem(pipedRdAddr)(i)(0))
                    poke(dut.io.rdPort.data(i).prio.n, mem(pipedRdAddr)(i)(1))
            ...
            if (peek(dut.io.wrPort.write) == 1) {
                for (i <- 0 until chCount) {
                    if ((peek(dut.io.wrPort.mask) & (BigInt(1) << i)) != 0) {
                        mem(pipedWrAddr)(i)(0) = peek(dut.io.wrPort.data(i).prio.c).toInt
                        mem(pipedWrAddr)(i)(1) = peek(dut.io.wrPort.data(i).prio.n).toInt
            ...
            if (peek(dut.io.headPort.write).toInt == 1) {
                root(0) = peek(dut.io.headPort.wrData.prio.c).toInt
                root(1) = peek(dut.io.headPort.wrData.prio.n).toInt
