...
class HeapPriorityQueue(
  ...
  val io = IO(new Bundle{
    ...
    val cmd = new Bundle{
      ...
      val rm_prio = Output(new Priority(nWid,cWid)) //TODO: requires additional read -> spare time during search phase
    }
  ...
  io.cmd.rm_prio := 0.U.asTypeOf(new Priority(nWid,cWid)) //TODO: we need to include an extra read...
  ...
  io.debug.swap := heapifier.io.swap
    ...
    is(resetCell){
      ...
      io.wrPort.mask := UIntToOH(rdIndexOffset)
      io.wrPort.write := true.B
