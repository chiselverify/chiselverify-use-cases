...
import priorityqueue.lib.Interfaces._
...
/**
  ...
  *
  */
class QueueControl extends Module {
  /////////////////////////////////////////////////////IO///////////////////////////////////////////////////////////////
  ...
  val headReg = RegInit(VecInit(Seq.fill(nWid + cWid + rWid)(1.U)).asTypeOf(new PriorityID))
  val tempReg = RegInit(VecInit(Seq.fill(childrenCnt)(0.U.asTypeOf(new PriorityID))))
  val removalIndex = RegInit(0.U(log2Ceil(size).W))
  val removedPrio = RegInit(0.U.asTypeOf(new Priority))
  ...
  val wrIndexToRam = ((wrIndex - 1.U) >> log2Ceil(childrenCnt)).asUInt
  val wrIndexOffset = Mux(wrIndex === 0.U, 0.U, wrIndex(log2Ceil(childrenCnt), 0) - 1.U(log2Ceil(size).W))

  // ram read address and offset
  val rdIndex = WireDefault(0.U(log2Ceil(size).W))
  val rdIndexToRam = ((rdIndex - 1.U) >> log2Ceil(childrenCnt)).asUInt
  val rdIndexOffset = Mux(rdIndex === 0.U, 0.U, rdIndex(log2Ceil(childrenCnt), 0) - 1.U(log2Ceil(size).W))

  val resetVal = VecInit(Seq.fill(nWid + cWid + rWid)(true.B)).asUInt.asTypeOf(new PriorityID)
  ...
  switch(stateReg) {
        ...
        when(io.cmd.op) { // insertion
          ...
          when(heapSizeReg =/= 0.U) {
            when(heapSizeReg === 1.U && headReg.id =/= io.cmd.refID) { //catch non matching reference ID on last removal
              errorReg := true.B
              stateReg := idle
              removedPrio := 0.U.asTypeOf(new Priority)
    ...
    is(normalInsertion) { // insertion into already filled queue
      ...
      io.wrPort.mask := UIntToOH(wrIndexOffset, childrenCnt)
      ...
      // initiate heapify up
      heapifier.io.control.idx := Mux(heapSizeReg < (childrenCnt + 1).U, 0.U, ((heapSizeReg - 1.U) >> log2Ceil(childrenCnt)).asUInt)
    ...
    is(resetCell) {
      ...
      io.wrPort.mask := UIntToOH(rdIndexOffset, childrenCnt)
      io.wrPort.write := true.B
    ...
    is(removal) {
      ...
      // initiate heapify up
      heapifier.io.control.heapifyUp := true.B
      heapifier.io.control.idx := Mux(removalIndex < (childrenCnt + 1).U, 0.U, ((removalIndex - 1.U) >> log2Ceil(childrenCnt)).asUInt)
      stateReg := waitForHeapifyUp
    }
    is(waitForHeapifyUp) { // wait for the heapifier to complete one up pass
      ...
      when(heapifier.io.control.done) {
        stateReg := idle
        when(io.cmd.op === 0.U && !heapifier.io.control.swapped && ((removalIndex << log2Ceil(childrenCnt)).asUInt + 1.U) < size.U) { // when no swap occurred during removal -> heapify down
