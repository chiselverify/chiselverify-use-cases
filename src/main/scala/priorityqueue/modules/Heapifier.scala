...
import priorityqueue.lib.Interfaces.{rdPort, wrPort}
...
/**
  ...
  * @param childrenCnt the number of children per node (must be power of 2)
  ...
  */
class Heapifier extends Module {
  val io = IO(new Bundle {
    ...
    // ram ports
    val rdPort = new rdPort
    val wrPort = new wrPort

    // port to the cached head element stored in a register
    val headPort = new Bundle {
      val rdData = Input(new PriorityID)
      val wrData = Output(new PriorityID)
  ...
  val minFinder = Module(new MinFinder(childrenCnt + 1)) // module to find the minimum priority among the parent and children
  minFinder.io.currentSC := 0.U //TODO
  ...
  val parentReg = RegInit(VecInit(Seq.fill(childrenCnt)(0.U.asTypeOf(new PriorityID)))) // register holding the content of the RAM cell containing the parent
  val childrenReg = RegInit(VecInit(Seq.fill(childrenCnt)(0.U.asTypeOf(new PriorityID)))) // register holding the content of the RAM cell of the children
  ...
  val ramAddressParent = ((addressIndex - 1.U) >> log2Ceil(childrenCnt)).asUInt() // the RAM address of the parent is calculated by (index-1)/childrenCount

  // parent selection
  val parentOffset = Mux(indexReg === 0.U, 0.U, indexReg(log2Ceil(childrenCnt), 0) - 1.U(log2Ceil(size).W)) // the offset of the parent within its RAM cell
  ...
  for (i <- 0 until childrenCnt) {
    minFinder.io.values(i + 1) := childrenReg(i)
  }

  val nextIndexUp = ((indexReg - 1.U) >> log2Ceil(childrenCnt)).asUInt() // index of next parent is given by (index-1)/childrenCount
  val nextIndexDown = (indexReg << log2Ceil(childrenCnt)).asUInt() + RegNext(minFinder.io.idx) // index of next parent is given by (index * childrenCount) + selected child
  ...
  io.wrPort.mask := VecInit(Seq.fill(childrenCnt)(true.B)).asUInt
  ...
  switch(stateReg) {
    ...
    is(wbDown2) { // update the index register and apply new index to address generation
      ...
      when((nextIndexDown << log2Ceil(childrenCnt)).asUInt >= io.control.heapSize) { // we have reached a childless index and can go to idle
        stateReg := idle
      }
    }
  }
